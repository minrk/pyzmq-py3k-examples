...
class HeartBeater(object):
    ...
    def beat(self):
        ...
        [self.handle_new_heart(h) for h in newhearts]
        [self.handle_heart_failure(h) for h in heartfailures]
        
        self.responses = set()
        print ("%i beating hearts: %s"%(len(self.hearts),self.hearts))
        self.msg = str(self.lifetime).encode('utf8')
        self.pingstream.send(self.msg)
